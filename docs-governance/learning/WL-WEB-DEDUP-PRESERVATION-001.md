...
## Addendum 2026-08-19 — intra-section redundancy and volatile facts

This addendum preserves the original learning record above and adds only the prevention required by the v16.5 review.

23. Run an **intra-section redundancy scan** on every rewritten section. If a concept is already expressed by a primary block, do not restate the same scope disclaimer or interpretation as multiple summary cards unless each repetition serves a distinct narrative function.
24. Apply **one concept, one primary home** inside a section as well as across the site. Measurement geography, benefit geography, downstream continuity and phase decisions must remain visually distinct rather than being repeated in interchangeable cards.
25. Do not duplicate a confluence, junction or shared node in parallel process lanes when that layout can imply two different places or sequential events. Use one shared convergence node when the underlying system has one confluence.
26. Treat monetary values, project budgets, capacities, schedules and similar facts as time-sensitive. Do not present an old numeric estimate without date/context as though it were current. When estimates change materially, either date them explicitly or keep the chronology in the evidence register and use stable narrative language on the website.
27. When place names are used as monitoring or benefit anchors, distinguish `orientation anchor`, `candidate station`, `benefit geography` and `confirmed site`. Do not convert settlement spacing into scientific station design without hydrological justification.
28. Unsupported geographic superlatives such as `exact midpoint` must be removed or downgraded unless verified. A reasonable intermediate location is not automatically an exact midpoint.

### Targeted regression — v16.5 revision

- the shared Huambutío confluence heading occurs once;
- the external href list remains unchanged during the content-only revision;
- the Stanley/WGA hard lock remains byte-identical;
- the old `estimated US$46M project` phrase is absent;
- the redundant headings `Primary working scope`, `Measurement can extend upstream`, `No scope inflation`, and `Capacity pressure` are absent from revised `#system`;
- the corridor uses Huarán and Pachar while keeping Yanahuara explicitly non-final;
- Huarán is not described as the exact midpoint.

Learning status for this addendum: IMPLEMENTED. Behavior verification remains tied to the exact v16.5 review surfaces and user approval gate.
